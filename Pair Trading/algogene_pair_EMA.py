...
class AlgoEvent:
        ...
        if isSync:
            if self.stock_x in bd and self.stock_y in bd:
                ...
                if bd[self.stock_x]['timestamp'] >= self.lasttradetime + timedelta(hours=24):
                    ...
                    if len(self.stock_one_prices) > self.window:
                        self.evt.consoleLog("EMA")
                        EMAX = talib.EMA(self.stock_one_prices, timeperiod=self.window)
                        EMAY = talib.EMA(self.stock_two_prices, timeperiod=self.window)
    ...
    def test_sendOrder(self, instrument, lastprice, buysell, openclose, z):
        ...
        if buysell==1:
            order.takeProfitLevel = lastprice*1.05
            order.stopLossLevel = lastprice*0.95
        elif buysell==-1:
            order.takeProfitLevel = lastprice*0.95
            order.stopLossLevel = lastprice*1.05
